...
def parse_lsblk():
	...
	lc_map = dict((k.lower(), v) for k, v in map.items())
...
def map_sata_wwn_to_hba_wwn():
	...
	for sd_dev in map_sd_to_sg:
		...
		Debug("map_sata_wwn_to_hba_wwn()::  Scanning sd_dev " + str(sd_dev))
		...
		Debug("map_sata_wwn_to_hba_wwn()::  wwn_list for " + str(sd_dev) + " = " + str(wwn_list))
...
def map_enclosures():
	...
	map_lsscsi = parse_lsscsi()

	print("DEBUGDEBUG:  map_lsscsi = " + str(map_lsscsi))

	map = {}

	for sd_dev, dict in map_lsscsi.items():

		if not re.search("enclosu", dict["type"]):
			continue
...
# map_lsblk will be the primary data structure of this script, and others will be used to augment it before printing
map_lsblk = parse_lsblk_disks()
Debug("main:: map_lsblk = " + str(map_lsblk))

# Parse "lsscsi" output and add useful fields to map_lsblk
map_sd_to_sg = parse_lsscsi()
Debug("main:: map_sd_to_sg = " + str(map_sd_to_sg))
