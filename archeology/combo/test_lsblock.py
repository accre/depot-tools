...
def SysExec(cmd):
        ...
        # If we don't have cached data, or it's too old, regenerate it
                ...
                try:
                        ...
                        cmd_list = cmd.split()
                CacheDataArray[cmd] = Popen(cmd_list, stdout=PIPE, stderr=STDOUT).communicate()[0]
                CacheTimeArray[cmd] = time.time()
                Return_Val = CacheDataArray[cmd]
...
def map_intermediate_SAS_to_WWN_with_storcli():
        ...
        Debug("map_intermediate_SAS_to_WWN_with_storcli():: function exit")
...
def map_intermediate_SAS_to_WWN_with_MegaCli():
        ...
        output_megacli = SysExec(MEGACLI_BIN + " -PDList -aALL -NoLog")
...
for i in Controller:
        ...
        exec_type = Get_SASController_ExecType(i)

        Debug("main():: Controller_Type == " + str(exec_type))

        if exec_type == "sas2ircu":
                sas2ircu_map = map_intermediate_SAS_to_WWN_with_sas2ircu()
                sas_offset = 0

        if exec_type == "megacli":
                megacli_map = map_intermediate_SAS_to_WWN_with_MegaCli()
                sas_offset = 0

        if exec_type == "storcli":
                storcli_map = map_intermediate_SAS_to_WWN_with_storcli()
                sas_offset = 1

        Controller_Type = exec_type
